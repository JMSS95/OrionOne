...
-   **Status:** Configured globally (10 req/min)
-   **Key Features:**
...
-   **Status:** Configured with role-based permissions
-   **Roles:** ADMIN, AGENT, USER
-   **Subjects:** Incident, User, KnowledgeBase, Comment
...
-   **Type:** Logging
-   **License:** MIT
-   **Purpose:** Structured logging with file rotation and custom transports
-   **Documentation:** [https://github.com/winstonjs/winston](https://github.com/winstonjs/winston) • [https://github.com/gremo/nest-winston](https://github.com/gremo/nest-winston)
-   **Levels:** error, warn, info, http, verbose, debug
-   **Status:** Configured with custom transports (console, file, error, exceptions)

**Configuration:**

```typescript
// nest-backend/src/config/logger.config.ts
...
export class IncidentsService {
    ...
    async createIncident(data: CreateIncidentDto) {
        this.logger.log("Creating incident", "IncidentsService");
        ...
        try {
            ...
            this.logger.error(
                `Failed to create incident: ${error.message}`,
                error.stack,
                "IncidentsService"
            );
            throw error;
